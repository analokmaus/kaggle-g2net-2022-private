...
EXPORT_DIR = Path(f'/scratch/acb10673fd/g2net2022/{DATASET}/')
...
def make_signal(gid):
    ...
    signal_kwargs = {
        "F0": freqs[0],
        "F1": np.random.choice([-1, 1], p=[0.9, 0.1]) * (10 ** np.random.uniform(F1_MIN, F1_MAX)),
        "F2": 0,
...
def generate_sample(index, test, iteration=0):
    ...
    np.random.RandomState(SEED_OFFSET+offset+index)
    np.random.seed(SEED_OFFSET+offset+index)
    ...
    with open(EXPORT_DIR/f'{test_id}_{iteration}.pickle', 'wb') as f:
        sft = sft['H1']*1e22
        pickle.dump({'spectrogram': (sft.real**2 + sft.imag**2).astype(np.float16), 'timestamps': ts['H1']}, f)
